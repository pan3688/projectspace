...
public class OptimisticBoostedPrioritySkipList implements IntSet {
	...
	public OptimisticBoostedPrioritySkipList() {
	...
	private boolean commitValidate(ArrayList<ReadSetEntry> readset) {
		...
		// Specific for PQ: Check that first min does not change
		PQThread t = ((PQThread) Thread.currentThread());
		if(t.firstMin != null && head.next[0] != t.firstMin)
			return false;
	...
	private boolean operation(int type, int item) throws AbortedException {
		TreeMap<Integer, WriteSetEntry> writeset = ((PQThread) Thread
		...
		ArrayList<ReadSetEntry> readset = ((PQThread) Thread.currentThread()).skiplist_readset;
