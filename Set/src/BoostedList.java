...
public class BoostedList implements IntSet {

	LazyList list = new LazyList();
	LockKey lock = new LockKey();
	final int LOCK_TIMEOUT = 10;  // in microseconds (large enough to reduce aborts and small enough no to hang)
...
//				lock.lock();
	...
	public boolean add(final int value) throws AbortedException {
		...
		if (result) {
			...
			((SetThread) Thread.currentThread()).list_log.push(e);
		}
		return result;
	}
	...
	@Override
	public void abort() {
		Stack<LogEntry> log = ((SetThread) Thread.currentThread()).list_log;
		while(!log.empty())
			log.pop().undo();
